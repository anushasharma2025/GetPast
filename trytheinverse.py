...
frustriggdelay = 1000
frusdur = 3000
WINNING_SCORE_LEVEL_2 = 10
WINNING_SCORE_LEVEL_3 = 10
...
def makecrop(level):
    ...
    speed = sprite_speed_base
    if level == 3:
        speed = random.uniform(sprite_speed_base * 1.5, sprite_speed_base * 2)
    elif level == 2:
        speed = random.uniform(sprite_speed_base - 1, sprite_speed_base * 0.8)
    else:
        speed = random.uniform(sprite_speed_base, sprite_speed_base * sprite_speed_multiplier)
...
def draw_ui(score1, score2, message=""):
    score1_text = sfont.render(f"P1 Score: {score1}", True, white)
    score1_rect = score1_text.get_rect(topright=(swiddth - 20, 20))
    screen.blit(score1_text, score1_rect)

    score2_text = sfont.render(f"P2 Score: {score2}", True, white)
    screen.blit(score2_text, (20, 20))

    if message:
        message_text = lfont.render(message, True, white)
...
def main():
    ...
    darkening_start_time = 0
    darkening_duration = 20000
    overlay_alpha = 0
    ...
    while True:
        ...
        if game_state.startswith("intro"):
            ...
            messages = {
                ...
                "intro_3": ("The zombie apocalypse is here.\nYour only hope for survival is the last thing you'd do.", 3000, dissonant_font_medium),
            ...
            if level == 2:
                rules_text_p1 = mfont.render("Player 1: Collect plants. (Arrow Keys) - BLUE", True, blue)
                rules_text_p2 = mfont.render("Player 2: Collect zombies. (WASD Keys) - PINK", True, pink)
                rules_text_sabotage = mfont.render("Fill your happiness meter for a circle of light!", True, white)
                rules_text_score = mfont.render("First to 10 points wins. Touching the wrong item reduces your score by 2.", True, white)
                rules_text_special = mfont.render("The game will get darker over time...", True, white)

                screen.blit(rules_text_p1, (swiddth // 2 - rules_text_p1.get_width() // 2, sheight // 2 - 50))
                screen.blit(rules_text_p2, (swiddth // 2 - rules_text_p2.get_width() // 2, sheight // 2 + 0))
                screen.blit(rules_text_sabotage, (swiddth // 2 - rules_text_sabotage.get_width() // 2, sheight // 2 + 50))
                screen.blit(rules_text_score, (swiddth // 2 - rules_text_score.get_width() // 2, sheight // 2 + 100))
                screen.blit(rules_text_special, (swiddth // 2 - rules_text_special.get_width() // 2, sheight // 2 + 150))
                
            if level == 3:
                rules_text_p1 = mfont.render("Player 1: Collect plants. (Arrow Keys) - BLUE", True, blue)
                rules_text_p2 = mfont.render("Player 2: Collect zombies. (WASD Keys) - PINK", True, pink)
                rules_text_sabotage = mfont.render("At score 5, smile at the camera. The player with the happiest face wins!", True, white)
                rules_text_inverted = mfont.render("The loser's controls will be inverted!", True, white)
                rules_text_score = mfont.render("First to 10 points wins. Touching the wrong item reduces your score by 2.", True, white)
                
                screen.blit(rules_text_p1, (swiddth // 2 - rules_text_p1.get_width() // 2, sheight // 2 - 70))
                screen.blit(rules_text_p2, (swiddth // 2 - rules_text_p2.get_width() // 2, sheight // 2 - 20))
                screen.blit(rules_text_sabotage, (swiddth // 2 - rules_text_sabotage.get_width() // 2, sheight // 2 + 30))
                screen.blit(rules_text_inverted, (swiddth // 2 - rules_text_inverted.get_width() // 2, sheight // 2 + 80))
                screen.blit(rules_text_score, (swiddth // 2 - rules_text_score.get_width() // 2, sheight // 2 + 130))


            continue_text = mfont.render("Click to Start!", True, white)
            continue_text_rect = continue_text.get_rect(center=(swiddth // 2, sheight // 2 + 200))
            screen.blit(continue_text, continue_text_rect)

            outline_rect = continue_text_rect.inflate(20, 10) 
            pygame.draw.rect(screen, white, outline_rect, 2) 
            
        elif game_state == "playing":
            
            if level == 2:
                if now - darkening_start_time < darkening_duration:
                    overlay_alpha = int(255 * (now - darkening_start_time) / darkening_duration)
                else:
                    overlay_alpha = 255
                
                overlay = pygame.Surface((swiddth, sheight), pygame.SRCALPHA)
                overlay.fill((0, 0, 0, overlay_alpha))

                if p1_light_on:
                    pygame.draw.circle(overlay, (0,0,0,0), p1_pos, 100)
                if p2_light_on:
                    pygame.draw.circle(overlay, (0,0,0,0), p2_pos, 100)
                screen.blit(overlay, (0,0))

            if level == 3:
                if (p1_score >= 5 or p2_score >= 5) and not level3_event_triggered:
                    game_state = "level3_event"
                    level3_event_triggered = True
            ...
            if p1_inverted and level == 3:
                inverted_text = sfont.render("CONTROLS INVERTED", True, red)
                screen.blit(inverted_text, (p1_pos[0] - inverted_text.get_width() // 2, p1_pos[1] + dinoblue_size // 2 + 5))
            if p2_inverted and level == 3:
                inverted_text = sfont.render("CONTROLS INVERTED", True, red)
                screen.blit(inverted_text, (p2_pos[0] - inverted_text.get_width() // 2, p2_pos[1] + deeno_size // 2 + 5))

            draw_ui(p1_score, p2_score)

            if level == 1 and (p1_score >= WINNING_SCORE or p2_score >= WINNING_SCORE):
                game_state = "game_over"
            if level == 2 and (p1_score >= WINNING_SCORE_LEVEL_2 or p2_score >= WINNING_SCORE_LEVEL_2):
                game_state = "game_over"
            if level == 3 and (p1_score >= WINNING_SCORE_LEVEL_3 or p2_score >= WINNING_SCORE_LEVEL_3):
                game_state = "game_over"
        
        elif game_state == "level3_event":
            screen.fill(black)
            smile_message = lfont.render("SMILE!", True, white)
            text_rect = smile_message.get_rect(center=(swiddth // 2, sheight // 2))
            screen.blit(smile_message, text_rect)
            
            current_time = pygame.time.get_ticks()
            if current_time - smile_start_time > 1000:
                ret, frame_for_analysis = cap.read()
                if ret:
                    frame_for_analysis = cv2.flip(frame_for_analysis, 1)
                    
                    webcam_frame_scaled = cv2.resize(frame_for_analysis, (webcam_rect_1.width * 2, webcam_rect_1.height))
                    webcam_frame_right_half = webcam_frame_scaled[:, webcam_frame_scaled.shape[1]//2:]
                    webcam_frame_left_half = webcam_frame_scaled[:, :webcam_frame_scaled.shape[1]//2]
                    ...
                    try:
                        analysis_p1 = DeepFace.analyze(webcam_frame_right_half, actions=['emotion'], enforce_detection=False, silent=True)
                        if analysis_p1 and 'happy' in analysis_p1[0]['emotion']:
                            p1_happy_score = analysis_p1[0]['emotion']['happy']
                    except Exception as e:
                        pass
                    
                    try:
                        analysis_p2 = DeepFace.analyze(webcam_frame_left_half, actions=['emotion'], enforce_detection=False, silent=True)
                        if analysis_p2 and 'happy' in analysis_p2[0]['emotion']:
                            p2_happy_score = analysis_p2[0]['emotion']['happy']
                    except Exception as e:
                        pass

                    # Determine winner and apply inversion
                    if p1_happy_score > p2_happy_score:
                        p2_inverted = True
                        p1_inverted = False
                    elif p2_happy_score > p1_happy_score:
                        p1_inverted = True
                        p2_inverted = False
                    else:
                        p1_inverted = False
            ...
            winner_text = ""
            if (level == 1 and p1_score >= WINNING_SCORE) or (level == 2 and p1_score >= WINNING_SCORE_LEVEL_2) or (level == 3 and p1_score >= WINNING_SCORE_LEVEL_3):
                winner_text = "Player 1 Wins!"
            elif (level == 1 and p2_score >= WINNING_SCORE) or (level == 2 and p2_score >= WINNING_SCORE_LEVEL_2) or (level == 3 and p2_score >= WINNING_SCORE_LEVEL_3):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if game_state == "level_select":
                    ...
                    continue_text_rect = mfont.render("Click to Start!", True, white).get_rect(center=(swiddth // 2, sheight // 2 + 200))
                    if continue_text_rect.collidepoint(event.pos):
                        ...
                        crops = [makecrop(level) for _ in range(initial_count)]
                        zombies = makezom(initial_count, level)
                        message = ""
                        p1_inverted = False
                        p2_inverted = False
                        level3_event_triggered = False
                        if level == 2:
                            darkening_start_time = pygame.time.get_ticks()
                            p1_light_on = False
                            p2_light_on = False
                elif game_state == "game_over":
        ...
        p2_emotion_text = sfont.render(f"P2 Mood: {p2_difficulty.capitalize()}", True, white)
        ...
        p1_emotion_text = sfont.render(f"P1 Mood: {p1_difficulty.capitalize()}", True, white)
