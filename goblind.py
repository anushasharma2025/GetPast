...
def draw_happiness_meter(meter_value, x, y):
    ...
    meter_text = sfont.render("Happiness", True, white)
...
def main():
    ...
    while True:
        ...
        if game_state.startswith("intro"):
            ...
            if level == 2:
                if level == 2 and now - darkening_start_time < darkening_duration:
                ...
                if p1_happiness_meter >= hmetermax and not p1_light_on:
                    p1_light_on = True
                    p1_happiness_meter = 0
                
                if p2_happiness_meter >= hmetermax and not p2_light_on:
                    p2_light_on = True
                    p2_happiness_meter = 0
            ...
            if level == 2:
                overlay_for_players = pygame.Surface((swiddth, sheight), pygame.SRCALPHA)
                overlay_for_players.fill((0, 0, 0, overlay_alpha))
                if p1_light_on:
                    pygame.draw.circle(overlay_for_players, (0,0,0,0), p1_pos, 100)
                if p2_light_on:
                    pygame.draw.circle(overlay_for_players, (0,0,0,0), p2_pos, 100)
                screen.blit(overlay_for_players, (0,0))
            ...
            winner_text = ""
            if (level == 1 and p1_score >= WINNING_SCORE) or (level == 2 and p1_score >= WINNING_SCORE_LEVEL_2):
                winner_text = "Player 1 Wins!"
            elif (level == 1 and p2_score >= WINNING_SCORE) or (level == 2 and p2_score >= WINNING_SCORE_LEVEL_2):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if game_state == "level_select":
                    ...
                    if continue_text_rect.collidepoint(event.pos):
                        ...
                        p1_score = 0
                        p2_score = 0
                        initial_count = 8 if level == 1 else 6
